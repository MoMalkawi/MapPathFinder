package dev.defiled.map.pathfinding.data;
...
public class Flags {
    ...
    public static boolean blockedNorth(int collisionData) {
        return Flags.checkFlag(collisionData, Flags.NORTH)
                || Flags.checkFlag(collisionData, Flags.BLOCKED_NORTH_WALL);
    }

    public static boolean blockedEast(int collisionData) {
        return Flags.checkFlag(collisionData, Flags.EAST)
                || Flags.checkFlag(collisionData, Flags.BLOCKED_EAST_WALL);
    }

    public static boolean blockedSouth(int collisionData) {
        return Flags.checkFlag(collisionData, Flags.SOUTH)
                || Flags.checkFlag(collisionData, Flags.BLOCKED_SOUTH_WALL);
    }

    public static boolean blockedWest(int collisionData) {
        return Flags.checkFlag(collisionData, Flags.WEST)
                || Flags.checkFlag(collisionData, Flags.BLOCKED_WEST_WALL);
    }

    public static boolean isFlagWalkable(int collisionData) {
        return !(Flags.checkFlag(collisionData, Flags.OCCUPIED)
                || Flags.checkFlag(collisionData, Flags.SOLID)
                || Flags.checkFlag(collisionData, Flags.BLOCKED)
                || Flags.checkFlag(collisionData, Flags.CLOSED));
    }

    public static int blockedByN(int collisionData) {
        int count = 0;
        if (blockedWest(collisionData)) count++;
        if (blockedNorth(collisionData)) count++;
        if (blockedSouth(collisionData)) count++;
        if (blockedEast(collisionData)) count++;
        return count;
    }


    private static final int UNWALKABLE = 256 | 0x200000 | 0x40000;

    public static boolean isFlagWalkable2(int flag) {
